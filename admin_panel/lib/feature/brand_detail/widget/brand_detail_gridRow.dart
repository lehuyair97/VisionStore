...
class BrandDetailGridRow extends DataGridSource {
  BrandDetailGridRow({required List<ProductBrand> products}) {
    final controller = Get.put(CreateProductController());
    _employees = products
        .map<DataGridRow>((e) => DataGridRow(cells: [
              DataGridCell<String>(
                  columnName: BrandDetailGridrow.id, value: e.id ?? ''),
              DataGridCell<String>(
                  columnName: BrandDetailGridrow.image, value: e.image ?? ''),
              DataGridCell<String>(
                  columnName: BrandDetailGridrow.name, value: e.name ?? ''),
              DataGridCell<double>(
                  columnName: BrandDetailGridrow.price, value: e.price ?? 0.0),
              DataGridCell<String>(
                  columnName: BrandDetailGridrow.introduce,
                  value: e.description ?? ''),
              DataGridCell<String>(
                  columnName: BrandDetailGridrow.brand,
                  value: controller.brandList
                      ...
                      ''),
              DataGridCell<String>(
                  columnName: BrandDetailGridrow.type,
              ...
              DataGridCell<IconData>(
                  columnName: BrandDetailGridrow.edit, value: Icons.edit),
              DataGridCell<IconData>(
                  columnName: BrandDetailGridrow.delete, value: Icons.delete),
            ]))
        .toList();
    ...
    return DataGridRowAdapter(
      ...
      cells: row.getCells().map<Widget>((dataGridCell) {
        ...
        switch (dataGridCell.columnName) {
          ...
          case BrandDetailGridrow.edit:
            return IconButton(
              onPressed: () {
                String productId = row
                    .getCells()
                    .firstWhere((cell) => cell.columnName == BrandDetailGridrow.id)
                    .value
                    .toString();
          ...
          case BrandDetailGridrow.delete:
            return IconButton(
              onPressed: () {
                String productId = row
                    .getCells()
